...
import { formatTime, USERS, createSocialNotification } from '../data';
...
const PostDetailModal = ({ postId, onClose, onOpenUser }) => {
  ...
  // 현재 로그인 유저가 게시글 작성자를 팔로우 중인지 추적
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'users', currentUser.uid), (snap) => {
      setIsFollowingAuthor(!!snap.data()?.following?.includes(post.userId));
    });
    ...
    try {
      ...
      if (!isLiked && post.userId !== user.uid) {
        ...
        await createSocialNotification({
          ...
          type: 'like',
          postId: post.id,
          postPreview: post.title || post.body || ''
        });
      }
    } catch (error) {
  ...
  const handleToggleFollowAuthor = async () => {
    ...
    const targetRef = doc(db, 'users', post.userId);

    try {
      if (isFollowingAuthor) {
        // 언팔로우
        await updateDoc(myRef, { following: arrayRemove(post.userId) });
        await updateDoc(targetRef, { followers: arrayRemove(currentUser.uid) });
      } else {
        // 팔로우
        await updateDoc(myRef, { following: arrayUnion(post.userId) });
...
// 댓글 & 대댓글 작성
  const handleAddComment = async () => {
    ...
    const safeComment = commentInput.trim();
    ...
    try {
      if (replyingTo) {
        ...
        await updateDoc(doc(db, 'posts', postId), {
          ...
          commentedUserIds: arrayUnion(user.uid)
        });
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isLoading ? (
          <div className="loading-spinner" style={{ padding: '80px 0', textAlign: 'center', color: '#888' }}>
            상세 내용을 불러오는 중... 🧭
          </div>
        ) : post ? (
          <div className="modal-scroll-container" style={{ flex: 1, overflowY: 'auto', paddingBottom: '20px' }}>
            ...
            <div className="modal-user-row" style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', gap: '12px', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, overflow: 'hidden' }}>
                <img
                  ...
                  onClick={() => onOpenUser && onOpenUser(post.userId)}
                />
                <div className="modal-user-info" style={{ display: 'flex', flexDirection: 'column' }}>
                  <div className="modal-username" style={{ fontWeight: 'bold', fontSize: '14px', color: '#1A1A1A', cursor: 'pointer' }} onClick={() => onOpenUser && onOpenUser(post.userId)}>
                    {userName}
                  </div>
                ...
                {!isMyPost ? (
                  <button
                    ...
                    style={{
                      padding: '6px 14px',
                      borderRadius: '20px',
                      border: isFollowingAuthor ? '1px solid #DEE2E6' : '1px solid #1A1A1A',
                      background: isFollowingAuthor ? '#F1F3F5' : 'transparent',
                      color: isFollowingAuthor ? '#495057' : '#1A1A1A',
                      fontSize: '12px',
                      fontWeight: 'bold',
                      cursor: 'pointer'
                    }}
              ...
              {post.tags && post.tags.length > 0 && (
                <div className="modal-tags" style={{ marginTop: '10px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {post.tags.map((tag, idx) => (
                    <span key={idx} className="post-tag" style={{ color: '#52B788', fontSize: '12px', fontWeight: '600' }}>
                      #{tag}
                    </span>
                  ))}
                </div>
                      ...
                      <div className="modal-photo-tag" style={{ position: 'absolute', bottom: '16px', left: '16px', backgroundColor: 'rgba(0,0,0,0.7)', color: '#fff', padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: 'bold', zIndex: 2 }}>
                        {isEditingName === idx ? (
                          <input
                            ...
                            onBlur={() => handleUpdateSpotName(idx)}
                            onKeyDown={(e) => e.key === 'Enter' && handleUpdateSpotName(idx)}
                            autoFocus
                            style={{ border: 'none', background: '#fff', color: '#000', borderRadius: '4px', padding: '1px 6px', outline: 'none', fontSize: '12px', fontWeight: 'bold' }}
                          />
                        ) : (
                ...
                return (
                  <div key={i} style={{ display: 'flex', flexDirection: 'column' }}>
                    <div className="timeline-row" style={{ display: 'flex', gap: '14px', padding: '10px 8px', borderRadius: '12px', backgroundColor: isCurrent ? '#F1F9F5' : 'transparent', transition: 'all 0.2s' }}>
                      <div className="node-badge" style={{ width: '22px', height: '22px', borderRadius: '50%', backgroundColor: isCurrent ? '#52B788' : '#ADB5BD', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 'bold', flexShrink: 0, marginTop: '2px' }}>
                        {i + 1}
                      </div>
                      <div style={{ flex: 1 }}>
            ...
            <div className="modal-actions" style={{ display: 'flex', gap: '20px', padding: '16px', borderTop: '1px solid #F1F3F5', fontSize: '18px' }}>
              ...
              <button className="modal-action-btn" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', color: '#495057', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <i className="far fa-comment"></i> {post.comments || 0}
              </button>
              <button className="modal-action-btn" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', color: '#495057', display: 'flex', alignItems: 'center', gap: '6px', marginLeft: 'auto' }}>
                <i className="far fa-bookmark"></i> {post.saves || 0}
              </button>
            </div>
                    ...
                    <div style={{ display: 'flex', gap: '12px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: 'flex', gap: '12px', marginTop: '6px' }}>
                          <button onClick={() => setReplyingTo({ commentId: comment.id, authorName: comment.authorName })} style={{ background: 'none', border: 'none', color: '#ADB5BD', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer', padding: 0 }}>
                            답글 달기
                          </button>
                          {auth.currentUser?.uid === comment.userId && (
                            <button onClick={() => handleDeleteComment(comment.id)} style={{ background: 'none', border: 'none', color: '#e63946', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer', padding: 0 }}>
                              삭제
                            </button>
                          )}
                        </div>
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              {auth.currentUser?.uid === reply.userId && (
                                <div style={{ marginTop: '4px' }}>
                                  <button onClick={() => handleDeleteReply(comment.id, reply)} style={{ background: 'none', border: 'none', color: '#e63946', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', padding: 0 }}>
                                    삭제
                                  </button>
                                </div>
                              )}
